...
void Generator::spawn(std::string type)
{
	...
	df::Vector pos((float)(rand() % (int)(WM.getBoundary().getHorizontal() - 10) + 5),
		(float)(rand() % (int)(WM.getBoundary().getVertical() - 6) + 3));
	...
	while (pos.getX() > sub->getPosition().getX() - 30 &&
		...
		pos.getY() < sub->getPosition().getY() + 7) 
	{
		pos = df::Vector((float)(rand() % (int)(WM.getBoundary().getHorizontal() - 10) + 5),
			(float)(rand() % (int)(WM.getBoundary().getVertical() - 6) + 3));
		counter--;
		if (counter < 0)
			break;
